...
import com.siterwell.sdk.action.GetHekrDataWithTokenListener;
import com.siterwell.sdk.action.HekrUserAction;
import com.siterwell.sdk.bean.JWTBean;
import com.siterwell.sdk.event.HttpErrorEvent;
...
public class HekrHttpUtil {
    ...
    private synchronized static void refreshToken(final Context context, String ReFresh_Token, final HekrHttpUtil.RefreshTokenListener refreshTokenListener) {
        ...
        String refresh_url = TextUtils.concat(ConstantsUtil.UrlUtil.BASE_UAA_URL, ConstantsUtil.UrlUtil.UAA_REFRESH_TOKEN).toString();
        BaseHttpUtil.postData(context, refresh_url, jsonObject.toJSONString(), new AsyncHttpResponseHandler() {

            @Override
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                Log.d(ConstantsUtil.HEKR_SDK, "token刷新成功:" + new String(bytes));
    ...
    private static void hekr_http(int http_type, final Context context, final String url, final Header[] headers, final String JWT_TOKEN, final String ReFresh_Token, final String entity, final RequestParams params, final GetHekrDataWithTokenListener getHekrDataListener) {
        if (TextUtils.isEmpty(JWT_TOKEN) || TextUtils.isEmpty(ReFresh_Token) || TextUtils.isEmpty(url)) {
            getHekrDataListener.getDataFail(ConstantsUtil.ErrorCode.TOKEN_TIME_OUT);
            Log.e(ConstantsUtil.HEKR_SDK_ERROR, "Token or url is null\n" + "token:" + JWT_TOKEN + "url\n" + url);
            logout(context);
        } else {
            if (Network.isConnected(context)) {
                switch (http_type) {
                    ...
                    case HTTP_POST_FILE:
                        ...
                        break;
                    default:
                        getHekrDataListener.getDataFail(ConstantsUtil.ErrorCode.UNKNOWN_ERROR);
                        break;
                }
            } else {
                Log.e(ConstantsUtil.HEKR_SDK_ERROR, url + "\n" + ConstantsUtil.NETWORK_ERROR);
                getHekrDataListener.getDataFail(ConstantsUtil.ErrorCode.NETWORK_TIME_OUT);
            }
        }
    }
}
