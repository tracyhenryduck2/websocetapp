...
import me.siter.sdk.entity.HekrWebBean;
import me.siter.sdk.inter.HekrWebActionListener;
import me.siter.sdk.web.HekrWebInterface;
...
public interface IHekrWebClient {
   ...
   * @param hekrWebBean  初始化HekrWeb需要的参数的bean
   */
  void initWithWeb(HekrWebInterface webInterface, View webView, HekrWebBean hekrWebBean);
   ...
   */
  void initWithWeb(HekrWebInterface webInterface, View webView, HekrWebBean hekrWebBean, String pushJsonMessage);
   ...
   */
  void initWithWeb(HekrWebInterface webInterface, View webView, HekrWebBean hekrWebBean, String pushJsonMessage, boolean useCache);
   ...
   * @param hekrWebBean  初始化HekrWeb需要的参数的bean
   */
  void init(HekrWebInterface webInterface, ViewGroup container, HekrWebBean hekrWebBean);
   ...
   * @param pushJsonMessage 是否需要有推从消息
   */
  void init(HekrWebInterface webInterface, ViewGroup container, HekrWebBean hekrWebBean, String pushJsonMessage);
   ...
   * @param useCache        是否开启页面内容的缓存。如果不开启，那么每次都会从服务器请求页面的所有内容。
   */
  void init(HekrWebInterface webInterface, ViewGroup container, HekrWebBean hekrWebBean, String pushJsonMessage, boolean useCache);

  /**
   * 加载相关页面，请在{@link #init(HekrWebInterface webInterface, ViewGroup viewGroup, HekrWebBean hekrWebBean, String pushJsonMessage, boolean isOpenWebCache) init}
   * 后调用。
   */
  void load();
   ...
   * @param pushJsonMessage 是否需要有推从消息
   */
  void reload(HekrWebBean hekrWebBean, String pushJsonMessage);

  /**
   * 设置Web页面的监听，主要监听Web中的一些动作。
   *
   * @param hekrWebActionListener 监听接口
   */
