...
import me.siter.sdk.connection.DeviceConnection;
...
class SiterDeviceClient implements ISiterDeviceClient, NetObservable, Connectable {

    private static final String TAG = SiterDeviceClient.class.getSimpleName();
    // 心跳包发送间隔时间
    ...
    private CopyOnWriteArrayList<SIterLANDeviceListener> mDeviceListeners;
    ...
    SiterDeviceClient(String tag, String ctrlKey) {
    ...
    @Override
    public synchronized void sendMessage(final JSONObject message, final SiterMsgCallback callback) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                tryToSend(message, new SiterMsgCallbackWrapper(callback));
            }
        });
    ...
    public synchronized void connect(final String ip, final int port) {
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                mNetMonitor.add(SiterDeviceClient.this);
                mConnectManager.add(SiterDeviceClient.this, getTag(), getConnType());
            }
        });
    }
    ...
    public synchronized void disconnect() {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                isOnline = false;
                isConnecting = false;
                mNetMonitor.remove(SiterDeviceClient.this);
                ...
                for (SIterLANDeviceListener listener : mDeviceListeners) {
    ...
    @Override
    public synchronized void addLANDeviceListener(SIterLANDeviceListener listener) {
        mDeviceListeners.add(listener);
    }

    @Override
    public synchronized void removeLANDeviceListener(SIterLANDeviceListener listener) {
...
//            }
    ...
    private synchronized void tryToBeat() {
        if (isOnline && mDeviceConnection != null && mDeviceConnection.isConnected()) {
            try {
                ...
                final MessageRequest request = new MessageRequest(addSpace(message.toString()), filter, new SiterMsgCallback() {
                    @Override
    ...
    private synchronized void tryToAuth() {
        ...
        try {
            ...
            MessageFilter filter = new MessageFilter(rule);
            MessageRequest request = new MessageRequest(addSpace(message.toString()), filter, new SiterMsgCallback() {
                @Override
                public void onReceived(String msg) {
    ...
    private class StatusListener implements ConnectionStatusListener {
        ...
        public void onSuccess() {
            SiterDeviceClient.this.onSuccess();
        }

        @Override
        public void onFail() {
            SiterDeviceClient.this.onFail();
        }

        @Override
        public void onConnected() {
            SiterDeviceClient.this.onConnected();
        }

        @Override
        public void onDisconnected() {
            SiterDeviceClient.this.onDisconnected();
        }

        @Override
        public void onError() {
            SiterDeviceClient.this.onError();
        }
    }
    ...
    private class SiterMsgCallbackWrapper implements SiterMsgCallback {

        private SiterMsgCallback msgCallback;

        SiterMsgCallbackWrapper(SiterMsgCallback msgCallback) {
