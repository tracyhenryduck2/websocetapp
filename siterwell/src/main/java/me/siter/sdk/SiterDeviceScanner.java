...
class SiterDeviceScanner implements ISiterDeviceScanner, ServiceListener {
    ...
    private SiterLANListener mLANListener;
    private ExecutorService mSingleThreadPool = Executors.newSingleThreadExecutor();

    SiterDeviceScanner() {
        @SuppressLint("WifiManagerLeak") WifiManager wifiMwanager = (WifiManager) SiterSDK.getContext().getSystemService(Context.WIFI_SERVICE);
    ...
    private synchronized void startSafely() {
        ...
        if (!NetworkUtil.isWifiConnected(SiterSDK.getContext())) {
            return;
        }
        ...
        try {
            ...
            final InetAddress deviceIpAddress = NetworkUtil.getDeviceIpAddress(SiterSDK.getContext());
            mJmdns = JmDNS.create(deviceIpAddress);
            mJmdns.addServiceListener(mType, SiterDeviceScanner.this);
    ...
    @Override
    public void addLANListener(SiterLANListener listener) {
