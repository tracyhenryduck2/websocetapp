 ...
 * {@link #login(String username, String password, SiterCallback callback) login}或者
 * {@link #login(String username, String password, SiterRawCallback callback) login}
 * 方法进行登录，登录成功后才能使用SDK的功能。
 * Created by TracyHenry on 2020/12/16.
 * Author: TracyHenry
 */
public interface ISiterUser {

  /**
   * 登录成功后获取token。
   *
   * @return 如果没有登录或者登录不成功，那么此方法返回空字符串。登录成功或者刷新token成功后返回token值，一段时间后请求
   * 服务器时token会失效，请使用{@link #refreshToken(SiterRawCallback callback) refreshToken()}
   * 刷新token。
   */
  String getToken();
   ...
   */
  void refreshToken(SiterRawCallback callback);
   ...
   * @see SiterCallback
   */
  void logout(SiterCallback callback);
}
