...
import me.siter.sdk.inter.HekrClientListener;
...
public class HekrCloudClient implements NetObservable, Connectable, AppStatusObservable {
    ...
    public void sendMessage(final JSONObject message, final HekrMsgCallback callback) {
    ...
    public void sendMessage(final String devTid, final JSONObject message, final HekrMsgCallback callback) {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    public synchronized void connect(final String url) {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                mNetMonitor.add(HekrCloudClient.this);
                mConnectManager.add(HekrCloudClient.this, getTag(), getConnType());
            }
        });
    }
    ...
    public synchronized void disconnect() {
        mHandler.post(new Runnable() {
            ...
            public void run() {
                isOnline = false;
                isConnecting = false;
                mNetMonitor.remove(HekrCloudClient.this);
                ...
                for (HekrClientListener listener : mHekrClientListeners) {
    ...
    private synchronized void tryToLogin() {
        try {
            ...
            params.put("token", Hekr.getHekrUser().getToken());
            ...
            final MessageRequest request = new MessageRequest(result.toString(),
                    new MessageFilter(filter), new HekrMsgCallback() {
    ...
    synchronized void addHekrClientListener(HekrClientListener listener) {
        mHekrClientListeners.add(listener);
    }

    synchronized void removeHekrClientListener(HekrClientListener listener) {
        mHekrClientListeners.remove(listener);
    }
    ...
    private synchronized void tryToBeat() {
        if (isOnline && mCloudConnection != null && mCloudConnection.isConnected()) {
            ...
            try {
                ...
                MessageRequest request = new MessageRequest(result.toString(), filter, new HekrMsgCallback() {
                    @Override
                    public void onReceived(String msg) {
    ...
    private void tryToSend(String devTid, JSONObject message, HekrMsgCallback callback) {
        tryToSend(message, callback);
    }
...
//            }
    ...
    private boolean checkStatus(JSONObject message, HekrMsgCallback callback) {
        if (message == null) {
